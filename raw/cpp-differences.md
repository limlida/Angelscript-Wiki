...
This behaviour can be turned off from Project Settings, if you prefer requiring `BlueprintCallable` to be explicit.
...
The angelscript integration follows this decision, meaning that when you declare a `float` in script, it is actually a 64-bit double-precision float. To create a floating-point variable with a specific width, you can explicitly use the `float32` or `float64` types.
