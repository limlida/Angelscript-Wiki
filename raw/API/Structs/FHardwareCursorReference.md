...
- `FName CursorPath [Specify the partial game content path to the hardware cursor.  For example,
...
Multi-Resolution Png Fallback
 ...
 on all platforms where pngs are all that is found instead of cur/ani/tiff.
   ...
   Pointer@2x.png
   ...etc]`
- `FVector2D HotSpot [HotSpot needs to be in normalized (0..1) coordinates since it may apply to different resolution images.
NOTE: This HotSpot is only used on formats that do not provide their own hotspot, e.g. Tiff, PNG.]`


**方法**:

- `FHardwareCursorReference& opAssign(FHardwareCursorReference Other)`
