...
- `TArray<FTakeRecorderTrackSettings> DefaultTracks [List of property names for which movie scene tracks will always record.]`
- `EUpdateClockSource RecordingClockSource [The clock source to use when recording]`
- `FDirectoryPath RootTakeSaveDir [The root of the directory in which to save recorded takes.]`
- `FString SubSequenceDirectory [The directory to record subscenes into.]`
- `FString TakeSaveDir [The name of the directory in which to save recorded takes. Supports any of the following format specifiers that will be substituted when a take is recorded:
...
- `bool bRecordSourcesIntoSubSequences [If enabled, each Source will be recorded into a separate Sequence and embedded in the Root Sequence will link to them via Subscenes track.
...
- `bool bRecordToPossessable [* If enabled, all recorded actors will be recorded to possessable object bindings in Sequencer. If disabled, all recorded actors will be
...
- `FTakeRecorderProjectParameters& opAssign(FTakeRecorderProjectParameters Other)`
