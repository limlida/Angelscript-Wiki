...
- `float32 CurveValueMinPrecision [Minimum delta allowed between consecutive sampled segments.
below this value, segments will be combined
to optimize number of markers.]`
- `float32 SamplingRate [Desired Sampling rate of above curve.
This will be rounded off so we sample the whole curve
with a fixed time step.]`
- `float32 Sum_dT_i_by_C_i [Cached Sum(dT_i * C_i)]`


**方法**:

- `FTimeStretchCurve& opAssign(FTimeStretchCurve Other)`
