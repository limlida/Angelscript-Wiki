...
- `EParticleAxisLock AxisLockOption [The axis to lock the mesh on. This overrides TypeSpecific mesh alignment as well as the LockAxis module.
...
- `EMeshCameraFacingOptions CameraFacingOption [The camera facing option to use:
...
- `float32 LODSizeScale [use the static mesh's LOD setup and switch LODs based on largest particle's screen size]`
- `UStaticMesh Mesh [The static mesh to render at the particle positions]`
- `EMeshScreenAlignment MeshAlignment [The alignment to use on the meshes emitted.
...
- `FRawDistributionVector RollPitchYawRange [The 'pre' rotation pitch (in degrees) to apply to the static mesh used.]`
- `bool bApplyParticleRotationAsSpin [If true, apply 'sprite' particle rotation about the orientation axis (direction mesh is pointing).
If false, apply 'sprite' particle rotation about the camera facing axis.]`
- `bool bCameraFacing [If true, then point the X-axis of the mesh towards the camera.
When set, AxisLockOption as well as all other locked axis/screen alignment settings are ignored.]`
- `bool bCollisionsConsiderPartilceSize [If true, all collisions for mesh particle on this emitter will take the particle size into account.
If false, particle size will be ignored in collision checks.]`
- `bool bEnableMotionBlur`
- `bool bFaceCameraDirectionRatherThanPosition [If true, all camera facing options will point the mesh against the camera's view direction rather than pointing at the cameras location.
If false, the camera facing will point to the cameras position as normal.]`
- `bool bOverrideDefaultMotionBlurSettings`
- `bool bOverrideMaterial [If true, use the emitter material when rendering rather than the one applied
to the static mesh model.]`
- `bool bUseStaticMeshLODs [use the static mesh's LOD setup and switch LODs based on largest particle's screen size]`
