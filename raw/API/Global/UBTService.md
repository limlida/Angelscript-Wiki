...
- `float32 Interval [defines time span between subsequent ticks of the service]`
- `float32 RandomDeviation [adds random range to service's Interval]`
- `bool bCallTickOnSearchStart [call Tick event when task search enters this node (SearchStart will be called as well)]`
- `bool bRestartTimerOnEachActivation [if set, next tick time will be always reset to service's interval when node is activated]`
