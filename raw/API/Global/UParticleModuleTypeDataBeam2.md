...
- `EBeam2Method BeamMethod [The method with which to form the beam(s). Must be one of the following:
...
- `FName BranchParentName [The name of the emitter to branch from (if mode is PEB2M_Branch)
MUST BE IN THE SAME PARTICLE SYSTEM!]`
- `FRawDistributionFloat Distance [The distance along the X-axis to stretch the beam
Distance is only used if BeamMethod is PEB2M_Distance]`
- `int InterpolationPoints [Indicates whether the beam should be interpolated.
    <= 0 --> no
    >  0 --> yes (and is equal to the number of interpolation steps that should be taken.]`
- `int MaxBeamCount [The number of live beams]`
- `bool RenderDirectLine`
- `bool RenderGeometry [Beam Rendering Variables.]`
- `bool RenderLines`
- `bool RenderTessellation`
- `int Sheets [The number of sheets to render]`
- `float32 Speed [The speed at which the beam should move from source to target when firing up.
    '0' indicates instantaneous]`
- `FRawDistributionFloat TaperFactor [Tapering factor, 0 = source of beam, 1 = target]`
- `EBeamTaperMethod TaperMethod [Tapering mode - one of the following:
...
- `FRawDistributionFloat TaperScale [Tapering scaling
...
- `int TextureTile [The number of times to tile the texture along each beam.
Overridden by TextureTilingDistance if it is > 0.0.
    1st UV set only. 2nd UV set does not Tile.]`
- `float32 TextureTileDistance [The distance per texture tile.
    1st UV set only. 2nd UV set does not Tile.]`
- `int UpVectorStepSize [The approach to use for determining the Up vector(s) for the beam.
...
- `bool bAlwaysOn [If true, there will ALWAYS be a beam...]`
