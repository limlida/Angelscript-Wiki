...
**Note:** If a property has an editable flag, but not a blueprint access flag, it will *only* be accessible in script from inside a class `default` statement. *See [Default Statements](https://angelscript.hazelight.se/scripting/actors-components/#default-statements).*
...
See [Overriding BlueprintEvents from C++](https://angelscript.hazelight.se/scripting/functions-and-events/#overriding-blueprintevents-from-c)
...
Note that for static functions only, the name of the class will go through [Namespace Simplification](https://angelscript.hazelight.se/scripting/function-libraries/#namespace-simplification) when they are bound.
