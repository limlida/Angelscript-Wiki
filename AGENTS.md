...
**未带 `trust` 字段的存量页面**：默认按 `partial` 处理（见 **4.4 查询决策流程**），但下次任何 agent 修改该页面时**必须补全 trust 字段**。
...
2. **检查 trust 戳**：读取相关 wiki 页面的 frontmatter，对每个用到的接口按 **4.4 查询决策流程** 决定是否回查 raw
   - 🟢 verified：直接引用，零回查
   - 🟡 partial / 无戳：检查 `unverified_items` 与方法旁标注，必要时回查
   - 🔴 unverified：必须回查 raw
3. **生成答案**：基于 wiki 内容生成回答，并引用来源页面；若发现页面错误或缺失，进入 **4.7 Demote/Promote**
4. **归档价值内容**：如果答案具有长期参考价值（如对比分析、教程），将其保存为新的综合页面到 `wiki/syntheses/`，并打 trust 戳（综合页面默认 🟡 起步，需 promote 才能 🟢）
5. **记录日志**：在 `log.md` 中记录查询和回答摘要
...
| 戳 | 含义 | 查询行为 |
|----|------|---------|
| 🟢 **verified** | 整页已从对应 `raw/API/<X>.md` 完整核对（方法名/参数/返回值都对照过） | **直接引用，零回查** |
...
- 凡是"两个接口怎么衔接"的逻辑（委托绑定/解绑配对、AbilityTask 工厂方法等），如果在 raw 中找不到完整示例，**必须打 `[TODO: verify]`**
...
| 来源情况 | 处置 |
|---------|------|
| `raw/API/{Global,Structs}/<X>.md` 有完整签名 | ✅ **可用**——直接引用并写入 wiki |
...
| 终态 | 触发条件 | 动作 |
|------|---------|------|
| ✅ **confirmed**（确认） | 在 `raw/API/{Global,Structs}/<X>.md` 找到完整签名；或用户实际运行代码确认成功；或外部权威来源（带链接）佐证 | 删除 `[TODO: verify]` 标记 → 添加 `来源：raw/...` 引用 → 必要时升级页面 trust → log.md 写 `verify` 条目 |
...
**动作**：
1. 通读该 wiki 页面，**逐项对照** `raw/API/<X>.md`
